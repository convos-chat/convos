(function(){Vue.component("convos-dialog-settings",{props:["dialog","user"],computed:{participants:function(){if(!this.dialog)return[];return this.dialog.participants()
.filter(function(p){return p.online})
.sort(function(a,b){return a.name.toLowerCase().localeCompare(b.name.toLowerCase());});}},methods:{shareUrl:function(){var dialog=this.dialog;return['','api','connection',dialog.connection_id,'dialog',dialog.dialog_id,'share']
