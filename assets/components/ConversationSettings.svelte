...
import {showConversationSettings, showParticipants} from '../store/viewport';
...
<div class="sidebar-right" transition:fly="{transition}">
  {#if $showConversationSettings}
    ...
    <form class="form-group" method="post" on:submit|preventDefault="{saveConversationSettings}">
      {#if !isPrivate}
  ...
  {#if $showParticipants}
    <nav class="participants">
      {#each participantsGrouped($participants.toArray()) as group}
        <h3>{$l(group.heading)} ({group.participants.length})</h3>
