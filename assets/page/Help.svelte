...
<main class="main">
  <p>
    {@html l.md('Got any questions? Come and talk to us in the "#convos" channel on https://freenode.net.')}
    {@html l.md('More information can also be found on https://convos.chat.')}
  </p>
  ...
  <dl>
    ...
    <dd>{@html l.md('Using "\*" around some text, will convert it into *italic*, **bold** or ***italic bold***.')}</dd>
    <dt>{l('`some fragment`')}</dt>
    <dd>{@html l.md('Using backticks around some text, will format it as a `code fragment`.')}</dd>
    <dt>https://...</dt>
    <dd>{l('URLs will be converted to links, and might be embedded in the chat.')}</dd>
  </dl>

  <h2 id="commands">{l('Available commands')}</h2>
  <p>{@html l.md('Any message starting with a-z is prefixed with "`/quote`", when sending from a connection conversation.')}</p>
  <dl>
