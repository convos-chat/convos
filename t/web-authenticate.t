...
$t->post_ok('/1.0/user/authenticate', json => {email => 'xyz', password => 'foo'})->status_is(400)
  ->json_is('/errors/0', {message => 'Does not match email format.', path => '/data/email'});

$t->post_ok('/1.0/user/authenticate', json => {email => 'superman@example.com'})->status_is(400)
  ->json_is('/errors/0/path', '/data/password');

$t->post_ok('/1.0/user/authenticate', json => {email => 'superman@example.com', password => 'xyz'})->status_is(400)
