...
is int @{$t->tx->res->json}, 60, 'got max limit messages';

$t->get_ok('/1.0/connection/IRC/localhost/conversation/%23convos/messages?limit=1')->status_is(200)
  ->json_is('/0/level',   'info')
  ->json_is('/0/message', 'The powernap package allows you to suspend servers which are not being used,')
  ->json_is('/0/sender',  'mr22')->json_is('/0/timestamp', '2015-06-22T10:23:50')->json_is('/0/type', 'privmsg')
  ->json_is('/1',         undef);

$t->get_ok('/1.0/connection/IRC/localhost/conversation/%23convos/messages?limit=3&match=AppArmor&level=warn')
  ->status_is(200)->json_is('/0/level', 'warn')
  ->json_is('/0/message', 'Unsure if AppArmor might be causing an issue? Don\'t disable it, use the')
  ->json_is('/0/sender',  'jhthorsen')->json_is('/0/timestamp', '2015-06-21T10:12:25')->json_is('/0/type', 'privmsg')
  ->json_is('/1',         undef);
