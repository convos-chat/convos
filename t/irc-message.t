...
$server->subtest(
  'not connected' => sub {
    my $err;
    $connection->send_p('', 'chanserv: whatever')->catch(sub { $err = shift })
      ->$wait_success('send_p');
    like $err, qr{Not connected}, 'target from message';

    $connection->send_p('#test_convos' => '0')->catch(sub { $err = shift })
      ->$wait_success('send_p');
...
$server->subtest(
  'notifications and unread' => sub {
    ...
    my $notifications;
    $core->get_user('superman@example.com')->notifications_p({})
      ->then(sub { $notifications = pop; })->$wait_success('notifications');
...
$server->subtest(
  'test stripping away invalid characters in a message' => sub {
...
$server->subtest(
  ...
  .*<superman>\syz\sa\sb\sc\r?\n
    ...
    require Convos::Plugin::Paste;
    $core->backend->on(message_to_paste => \&Convos::Plugin::Paste::_message_to_paste_p);
...
$server->subtest(
  'allow sending a single space' => sub {
...
$server->subtest(
  'paste' => sub {
    ...
    local *Convos::Core::Connection::Irc::_irc_event_privmsg
      = sub { push @msg, pop(@_)->{raw_line} };
