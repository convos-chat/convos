...
$dialog->messages({before => "2014-06-30T00:00:00"}, sub { ($err, $messages) = @_[1, 2]; Mojo::IOLoop->stop; });
Mojo::IOLoop->start;
...
$dialog->messages({before => "2014-06-21T14:30:00"}, sub { ($err, $messages) = @_[1, 2]; Mojo::IOLoop->stop; });
Mojo::IOLoop->start;
...
$dialog->messages({after => "2014-06-30T00:00:00"}, sub { ($err, $messages) = @_[1, 2]; Mojo::IOLoop->stop; });
Mojo::IOLoop->start;
...
$dialog->messages({after => "2014-06-21T14:30:00"}, sub { ($err, $messages) = @_[1, 2]; Mojo::IOLoop->stop; });
Mojo::IOLoop->start;
...
$dialog->messages({before => "2014-04-01T00:00:00", after => "2014-04-01T00:00:00"}, sub { ($err, $messages) = @_[1, 2]; Mojo::IOLoop->stop; });
Mojo::IOLoop->start;
is int @$messages, 0, 'before and after: same' or diag $err;

$dialog->messages({before => "2014-04-01T00:00:00", after => "2015-04-01T00:00:00"}, sub { ($err, $messages) = @_[1, 2]; Mojo::IOLoop->stop; });
Mojo::IOLoop->start;
is int @$messages, 0, 'before and after: "after" after "before"' or diag $err;

$dialog->messages({before => "2015-04-01T00:00:00", after => "2013-04-01T00:00:00"}, sub { ($err, $messages) = @_[1, 2]; Mojo::IOLoop->stop; });
Mojo::IOLoop->start;
is int @$messages, 0, 'before and after: difference > 12 months' or diag $err;

$dialog->messages({before => "2013-04-01T00:00:00", after => "2015-04-01T00:00:00"}, sub { ($err, $messages) = @_[1, 2]; Mojo::IOLoop->stop; });
Mojo::IOLoop->start;
is int @$messages, 0, 'before and after: difference > 12 months (2)' or diag $err;

$dialog->messages({match => 'iotop', after => "2014-08-01T00:00:00"}, sub { ($err, $messages) = @_[1, 2]; Mojo::IOLoop->stop; });
Mojo::IOLoop->start;
is int @$messages, 2, 'two messages matching iotop' or diag $err;
is $messages->[0]{ts}, '2014-08-21T10:13:32', 'first: 2014-08-21T10:13:32';

$dialog->messages({limit => 2, match => qr{\bpacka\w+\b}, after => "2014-08-01T00:00:00"},
