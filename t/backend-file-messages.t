...
subtest 'limit=1' => sub {
  $t->get_ok('/api/connection/irc-localhost/conversation/%23convos/messages?limit=1')
    ->status_is(200)->json_is('/after', undef)
    ->json_is('/before', $t->tx->res->json->{messages}[0]{ts})->json_is('/messages/0/from', 'toad')
    ->json_is('/messages/0/message', '80 bead');
  num_messages_is($t, 1, 'limit=1');
};

subtest 'before=2018-01-01T00:39:00' => sub {
  $t->get_ok(
...
subtest "before=$before, limit=10" => sub {
...
subtest "after=$after, limit=29" => sub {
...
subtest "around=$after, limit=10" => sub {
  $t->get_ok(
    "/api/connection/irc-localhost/conversation/%23convos/messages?around=$around&limit=10")
    ->status_is(200)->json_is('/after', $t->tx->res->json->{messages}[-1]{ts})
    ->json_is('/before',           $t->tx->res->json->{messages}[0]{ts})
...
subtest "around=$after, few messages" => sub {
...
subtest 'conversation name with slash' => sub {
