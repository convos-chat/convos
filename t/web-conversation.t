...
$t->post_ok('/1.0/connection/irc/localhost/conversations', json => {name => '#convos'})->status_is(401);
$t->post_ok('/1.0/user/login', json => {email => 'superman@example.com', password => 's3cret'})->status_is(200);

$t->post_ok('/1.0/connection/irc/localhost/conversations', json => {name => "#c"})->status_is(400)
  ->json_has('/errors/0/message');

$t->post_ok('/1.0/connection/irc/localhost/conversations', json => {name => '#convos'})->status_is(400)
...
$t->post_ok('/1.0/connection/irc/localhost/conversations', json => {name => '#convos'})->status_is(200)
