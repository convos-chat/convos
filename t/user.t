...
$t->post_ok('/register', form => {login => '1', password => '1', email => '1'})->status_is('200')
  ->element_exists('.error')->element_exists('div[title*="Username must consist"]')
  ->element_exists('div[title*="Invalid email"]')->element_exists('div[title*="same password twice"]');

$t->post_ok('/register', form => {login => 'foobar', email => 'foobar@barbar.com', password => ['barbar', 'barbar'],})
  ->status_is('302');

$t->ua->cookie_jar->empty;    # logout

# 'same user tries to log in later on...';
$t->post_ok('/login', form => {login => 'foobar', password => 'barbar'})->status_is('302');

$t->ua->cookie_jar->empty;    # logout

# 'new user tries to log in...';
$t->post_ok('/register', form => {})->status_is(200, 'Second user needs an invite code')->element_exists('.error')
...
)->status_is('200')->element_exists('.error')->element_exists('div[title*="Username is taken"]');
