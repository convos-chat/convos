...

{
  ...
  $dom->parse($t->message_ok->message->[1]);
  ok $dom->at('li.nicks[data-server="wirc.pl"][data-target="#wirc"]'), 'Got correct 6+#wirc';
...

{
  ...
  $dom->parse($t->message_ok->message->[1]);
  ok $dom->at('li.server-message.error[data-server="wirc.pl"][data-target="any"]'), 'Got IRC error';
...

{
  ...
  ok $dom->at('li.server-message.notice[data-server="wirc.pl"][data-target="any"]'), 'Got server message';
...

{
  ...
  $dom->parse($t->message_ok->message->[1]);
  ok $dom->at('li.whois[data-server="wirc.pl"][data-target="any"]'), 'Got whois';
...

{
  ...
  $dom->parse($t->message_ok->message->[1]);
  ok $dom->at('li.whois[data-server="wirc.pl"][data-target="any"]'), 'Got whois channels';
...

{
  ...
  $dom->parse($t->message_ok->message->[1]);
  ok $dom->at('li.topic[data-server="wirc.pl"][data-target="#wirc"]'), 'Got no topic';
...

{
  ...
  $dom->parse($t->message_ok->message->[1]);
  ok $dom->at('li.nick-change[data-server="wirc.pl"][data-target="any"]'), 'nick change';
...

{
  ...
  $dom->parse($t->message_ok->message->[1]);
  ok $dom->at('li.nick-parted[data-server="wirc.pl"][data-target="#mojo"]'), 'user parted';
  is $dom->at('div.content')->all_text, 'fooman parted #mojo', 'fooman parted #mojo';

  $connection->irc_part({ params => [ '#mojo' ], prefix => 'doe!user@host' });
  $dom->parse($t->message_ok->message->[1]);
  ok $dom->at('li.remove-conversation[data-server="wirc.pl"][data-target="#mojo"]'), 'self parted';
}

{
  $connection->irc_err_bannedfromchan({ params => [ 'doe', '#mojo', 'Cannot join channel (+b)' ] });
  $dom->parse($t->message_ok->message->[1]);
  ok $dom->at('li.remove-conversation[data-server="wirc.pl"][data-target="#mojo"]'), 'remove conversation when banned';
...

{
  ...
  $dom->parse($t->message_ok->message->[1]);
  ok $dom->at('li.notice[data-server="wirc.pl"][data-target="#wirc"]'), 'mode to #wirc';
