...
{
  $t->websocket_ok('/socket')->send_ok('<div data-server="magnet" data-target="#test123" id="003cb6af-e826-e17d-6691-3cae034fac1a">/names</div>');
  $dom->parse($t->message_ok->message->[1]);
  ok $dom->at('li.nicks[data-server="magnet"][data-target="#convos"]'), 'Got correct 6+#convos';
...

{
  ...
  $connection->add_message({
    params => [ '#mojo', 'http://convos.by is really cool' ],
    prefix => 'fooman!user@host',
  });
  $dom->parse($t->message_ok->message->[1]);
  is eval { $dom->at('a[href="http://convos.by/"]')->text }, 'http://convos.by/', 'not with "is really cool"' or diag $dom;
...

{
  ...
  $connection->add_message({
    params => [ '#mojo', 'doe: see this &amp; link: http://magnet/foo really cool' ],
    prefix => 'fooman!user@host',
  });
  $dom->parse($t->message_ok->message->[1]);
  ...
  $connection->add_message({
    params => [ '#mojo', 'doe: see this &amp; link: http://magnet/foo' ],
    prefix => 'fooman!user@host',
  });
  $dom->parse($t->message_ok->message->[1]);
...

{
  ...
  $dom->parse($t->message_ok->message->[1]);
  ok $dom->at('li.server-message.error[data-server="magnet"][data-target="any"]'), 'Got IRC error';
...

{
  ...
  ok $dom->at('li.server-message.notice[data-server="magnet"][data-target="any"]'), 'Got server message';
...

{
  $connection->irc_rpl_whoisuser({
    params => ['', 'doe', 'john', 'magnet', '', 'Real name'],
  });
  $dom->parse($t->message_ok->message->[1]);
  ok $dom->at('li.whois[data-server="magnet"][data-target="any"]'), 'Got whois';
...

{
  ...
  $dom->parse($t->message_ok->message->[1]);
  ok $dom->at('li.whois[data-server="magnet"][data-target="any"]'), 'Got whois channels';
...

{
  ...
  $dom->parse($t->message_ok->message->[1]);
  ok $dom->at('li.topic[data-server="magnet"][data-target="#convos"]'), 'Got no topic';
  is $dom->at('div.content')->all_text, 'No topic is set.', 'No topic is set';

  $connection->irc_topic({ params => ['#convos', 'Awesome'] });
  $dom->parse($t->message_ok->message->[1]);
  ok $dom->at('li.topic[data-server="magnet"][data-target="#convos"]'), 'Got topic';
...

{
  ...
  $dom->parse($t->message_ok->message->[1]);
  ok $dom->at('li.nick-joined[data-server="magnet"][data-target="#mojo"]'), 'user joined';
...

{
  ...
  $dom->parse($t->message_ok->message->[1]);
  ok $dom->at('li.nick-change[data-server="magnet"][data-target="any"]'), 'nick change';
...

{
  ...
  $dom->parse($t->message_ok->message->[1]);
  ok $dom->at('li.nick-parted[data-server="magnet"][data-target="#mojo"]'), 'user parted';
  is $dom->at('div.content')->all_text, 'fooman parted #mojo', 'fooman parted #mojo';

  $connection->irc_part({ params => [ '#mojo' ], prefix => 'doe!user@host' });
  $dom->parse($t->message_ok->message->[1]);
  ok $dom->at('li.remove-conversation[data-server="magnet"][data-target="#mojo"]'), 'self parted';
}

{
  ...
  $dom->parse($t->message_ok->message->[1]);
  ok $dom->at('li.error[data-server="magnet"][data-target="any"]'), 'magnet got banned error';
  ...
  $dom->parse($t->message_ok->message->[1]);
  ok $dom->at('li.remove-conversation[data-server="magnet"][data-target="#mojo"]'), 'remove conversation when banned';
}
...

{
  ...
  $dom->parse($t->message_ok->message->[1]);
  ok $dom->at('li.notice[data-server="magnet"][data-target="#convos"]'), 'mode to #convos';
...

{
  ...
  $dom->parse($t->message_ok->message->[1]);
  ok $dom->at('li.nick-quit[data-server="magnet"][data-target="any"]'), 'nick quit';
