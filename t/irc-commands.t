...
$server->subtest(
  'ison' => sub {
    ...
    $res = Mojo::Promise->all(map { $connection->send_p('', "/ison $_") }
        qw(SuperBoy superwoman SUPERBAD))->then(sub { [map { $_->[0] } @_] })->$wait_success;
    $server->processed_ok;
    is_deeply(
      $res,
      [
