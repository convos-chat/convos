;(function($) {
  ...
  var initSocket = function() {
    ...
    $input.send = function(message, history) {
      ...
      if(!message.match('^\/')) {
        var $pendingMessage = $('<li class="message-pending"><div class="content">' + message + '</div></li>').attr('id', uuid).cidAndTarget($messages);
        setTimeout(function() { messageFailed($pendingMessage); }, 10000);
        $pendingMessage.appendToMessages();
        $win.scrollTo('bottom');
      }
      $input.socket.send($('<div/>').cidAndTarget($messages).attr('id', uuid).attr('data-history', history).text(message).prop('outerHTML'));
