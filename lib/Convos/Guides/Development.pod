...
Once you have the source code you should install the dependencies:
  ...
  $ cpanm CSS::Minifier::XS CSS::Sass JavaScript::Minifier::XS
  $ npm install riot
  $ npm info riot # see which version is installed

Note! It seems like riot 2.3.1 has a bug which will cause failure. Please
edit "node_modules/riot/lib/server/index.js" and comment out line 14:18 -
meaning the body of the "extensions tag" function.

L<CSS::Sass> is not required if the C<npm> or C<sass> executable is already
...
be used to generate the "public" files. The dialogue is done with
...
=head2 Convos core
  ...
  | Core |
  '------'   .------.   .-------------.   .-----------.
      \______| User |___| Connections |___| Dialogues |
             '------'   '-------------'   '-----------'

L<Convos::Core> is the heart of L<Convos>. The core takes care of connections,
dialogues, can persist to a backend and provide hooks for plugins.
...
as well as keeping any number of dialogues active.

The way the "Backend" is hooked into the rest of the object graph is by
events. Any user, connection or dialogue can emit new events that the
...
Add start up script/command, so it can be deployed properly.
...
L<Convos::Core::Connection::Irc> need to understand commands, such as "/me",
"/join", "/part", "/nick", ...
...
looked at the dialogue. Emit desktop notifications on mentions.
...
Show user list for a dialogue and user count. (what about user modes?)

Group dialogues by connection.

Sort dialogues by name/last used/...?
...
Display topic when joining a dialogue the first time.

=item * Clean up git history

Want to remove all vendor files in git history, since they are not used after
changing to AssetPack.
