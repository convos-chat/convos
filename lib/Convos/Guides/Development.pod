...
NOTE! The current development branch is "one-point-oh":
...
The command above is the same as:
  ...
  $ MOJO_IRC_DEBUG=1 CONVOS_DEBUG=1 \
    morbo script/convos -w assets/sass -w assets/js -w public/convos-api.json
...
L<http://vuejs.org>. This application gets its data from a L<Swagger2>
...
"one-point-oh" branch is starting to take form. The API is pretty usable and
...
=item * Vuejs based user interface
