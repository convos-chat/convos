...
L<CSS::Sass> is not required if the C<sass> executable is already available
on your system. Have a look at
L<Mojolicious::Plugin::AssetPack::Preprocessor::Scss/DESCRIPTION> for more
details.
...
L<Convos::Core> need to be able to connect L<Convos::Core::Connections>
objects with the state "connecting". This need to be done serialized
(grouped by server), so we don't cause "flood connect".
...
looked at the conversation. Emit desktop notifications on mentions.
...
Show user list for a conversation and user count. (what about user modes?)

Group conversations by connection.

Sort conversations by name/last used/...?
...
Display topic when joining a conversation the first time.
