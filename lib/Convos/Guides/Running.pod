...
You can start convos by running the command below:
  ...
  $ cd convos
  $ git checkout -b one-point-oh origin/one-point-oh
...
L<Convos> can be configured with either a config file or environment variables.
...
=item * CONVOS_FORCED_IRC_SERVER

Will force any L<Convos::Controller::Connection/create> to use this server
instead of whatever was posted in. Note that this environment variable also
overrride L</CONVOS_DEFAULT_SERVER>.
...
by setting C<CONVOS_INVITE_CODE="">.
...
L<Convos::Core> requires shared memory, which a forked environment contradicts.
