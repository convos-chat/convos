...
Convos::Manual::Run - How to start Convos

=head1 DESCRIPTION

L<Convos> can run in different modes, but we suggest running with a seperate
...
=head2 Seperate backend

Starting up the backend seperately is a big win, since it allows you to
...
The backend can then be instructed with environment variables from
...
Run the same command again, and the webserver will
L<hot reload|http://mojolicio.us/perldoc/Mojo/Server/Hypnotoad#USR2> the source
code without loosing any connections.
...
set, so you can change the environment during L<hot reload|Mojo::Server::Hypnotoad/DESCRIPTION>.
  ...
  $ MOJO_CONFIG=/path/to/config.pl hynotoad script/convos

L</Hypnotoad> mode require the backend to be started as a seperate process.
